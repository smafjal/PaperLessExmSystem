...
public class TeachesApiImpl implements TeachesApi {
    ...
    public TeachesBean addTeaches(TeachesBean model) throws GenericBusinessException {
        Teaches teaches = new TeachesBean().toEntity(model);
        TeachesService service = new TeachesService();
        service.addTeaches(teaches);
        return getTeaches(teaches.getPrimaryKey());
    }
    ...
    @Override
    public TeachesBean CheckTeaches(int teachersId, int courseId) throws GenericBusinessException {
        TeachesBean teachesBean = new TeachesBean();
        Integer tecId=teachersId;
        ArrayList<TeachesBean> teachesBeans = (ArrayList<TeachesBean>) this.findTeachesByTeachersId(teachersId);
        
        
        //List<TeachesBean> teachesBeans = new ArrayList<>();
        
        for (int i = 0; i < teachesBeans.size(); i++) {
            
            System.out.println("Tec: "+i+" -- "+teachesBeans.get(i).getCourseId().getCourseId());
            
            if (teachesBeans.get(i).getCourseId().getCourseId() == courseId) {
                teachesBean = teachesBeans.get(i);
                break;
            }
        }
        return teachesBean;
