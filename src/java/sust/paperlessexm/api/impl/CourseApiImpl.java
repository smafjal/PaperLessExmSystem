...
public class CourseApiImpl implements CourseApi {
    ...
    @Override
    public CourseBean addCourse(CourseBean model) throws GenericBusinessException {

        Course course = new CourseBean().toEntity(model);
        CourseService courseService = new CourseService();
        courseService.addCourse(course);
        return getCourse(course.getPrimaryKey());
    }
    ...
    @Override
    public CourseBean checkCourse(String courseCode, int session) throws GenericBusinessException {

        CourseBean courseBean = new CourseBean();
        CourseApiImpl courseApiImpl = new CourseApiImpl();
        List<CourseBean> courseBeanList = courseApiImpl.findCourseBySession(session);

        for (int i = 0; i < courseBeanList.size(); i++) {
            if (courseCode.contains(courseBeanList.get(i).getCourseCode())) {
                courseBean = courseBeanList.get(i);
                break;
            }
        }
        return courseBean;
