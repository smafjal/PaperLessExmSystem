...
public class UpdateTable {
    ...
    public static void main(String[] args) throws GenericBusinessException {
        ...
        for (int i = 11; i < Math.min(50,listCourse.size()); i++) {
            for (int j = 0; j < Math.min(50,listTeachers.size()); j++) {

                if (listCourse.get(i).getCourseId() == 22 && listTeachers.get(i).getTeachersId() == 22) {
                    continue;
                }

                teachesBean.setCourseId(listCourse.get(i));
                teachesBean.setTeachersId(listTeachers.get(j));
                teachesBean.setTeachesId(null);

                //System.out.println("C_id: " + listCourse.get(i).getCourseId() + " T-id: " + listTeachers.get(j).getTeachersId());
...
//        }
        ...
        System.out.println("Str: " + listst.get(0).getStudentName());
